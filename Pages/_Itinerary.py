...
conn = get_connection()
active_trip = pd.read_sql("SELECT start_date FROM trips WHERE id = %s;", conn, params=(active_trip_id,)).iloc[0]
conn.close()

with st.form("add_itinerary_form", clear_on_submit=True):
    i1, i2 = st.columns(2)
    day_date = i1.date_input("Activity Date", value=active_trip["start_date"])
